...
public class StringGetBytesExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String str2 = new String(bytes2, "UTF-8");	// EUC-KR은 한글을 2바이트 취급한다. 깨짐. 문자셋이 다르기 때문
			System.out.println("바이트 => 문자열 : " + str2);
			
			byte[] bytes3 = str.getBytes("UTF-8");
			System.out.println("bytes3.length : " + bytes3.length);	//15바이트 이유는?
			
			String str3 = new String(bytes3, "UTF-8");		//UTF-8 문자셋은 한글 한자를 3바이트로 취급
